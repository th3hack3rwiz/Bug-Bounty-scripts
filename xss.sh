...
rm gf_xss
cd xss
cat ../${1}.aliveURLs| grep -v FUZZ | sed 's/,,.*//g' | sed 's/,/\n/g' | grep ${1} | sed 's/^.*http/http/g' | grep "=" | egrep -iv ".(js|jpg|woff|pdf|png|jpeg|gif|css|tif|tiff|ttf|woff2|ico|svg|txt)" | qsreplace -a | qsreplace | kxss |anew -q kxsss 
