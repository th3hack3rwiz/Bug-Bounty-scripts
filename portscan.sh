...
rm scan.txt
for i in $(cat port_ip | awk '{print $2}' | grep -o -E "([0-9]{1,3}[\.]){3}[0-9]{1,3}" |sort -u); do ports=$(cat port_ip | grep $i | awk '{print $1}' | sort -u | xargs |>
file=nmapme
while read i
 do
  ...
  #echo $port $ip
  sudo nmap -sV -Pn -A -p $port $ip -sC -T4 --script vulners.nse | tee -a nmap_results
done < $file 
